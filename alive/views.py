...
from pyview.template import template_file, LiveRender, LiveTemplate
...
def _load_template(filename):
    """Load a template by name using the configured ibis loader (supports multi-dir)."""
    from pyview.vendor import ibis
    template = ibis.loader(filename)
    return LiveTemplate(template)


def _render_with_extra(template, context, meta):
    """Create a LiveRender that merges context.extra into top-level template variables."""
    merged = _asdict(context)
    extra = merged.pop("extra", {})
    merged.update(extra)
    return LiveRender(template, merged, meta)
...
class ModelContext:
    ...
    frame: dict = field(default_factory=dict)
    # Extension data from app hooks (merged into template context at render time)
    extra: dict = field(default_factory=dict)
...
def create_model_liveview(model: Type[models.Model], url_prefix: str = "/alive") -> Type[LiveView]:
    ...
    class GeneratedModelLiveView(EditableFieldMixin, ItemMixin, LiveView[ModelContext]):
        ...
        async def render(self, context: ModelContext, meta: PyViewMeta):
            ...
            if conf.template:
                return _render_with_extra(_load_template(conf.template), context, meta)
            if context.view_mode == "grid":
                return _render_with_extra(template_file(GRID_TEMPLATE_PATH), context, meta)
            return _render_with_extra(template_file(DEFAULT_TEMPLATE_PATH), context, meta)
        ...
        async def mount(self, socket: LiveViewSocket[ModelContext], session: dict[str, Any]):
            ...
            ContextClass = conf.context_class or ModelContext
            socket.context = ContextClass(
                items=[],
            ...
            # Call app mount hook
            if conf.mount_hook:
                await conf.mount_hook(socket, session)
            ...
            if is_connected(socket):
                ...
                # Subscribe to app-provided extra channels
                if conf.extra_subscriptions:
                    for channel in await conf.extra_subscriptions(socket):
        ...
        async def handle_params(self, url, params: dict, socket: LiveViewSocket[ModelContext]):
            ...
            # Call app params hook
            if conf.params_hook:
                await conf.params_hook(socket, url, params)
        ...
        async def disconnect(self, socket: LiveViewSocket[ModelContext]):
            ...
            if conf.disconnect_hook:
                await conf.disconnect_hook(socket)
        ...
        async def handle_event(self, event: str, payload: dict[str, Any], socket: LiveViewSocket[ModelContext]):
            # App event handler gets first crack; if it returns True, skip built-in handling
            if conf.event_handler:
                if await conf.event_handler(event, payload, socket):
                    return
            ...
            if event == "save_create":
                ...
                # Call app post-create hook
                if conf.post_create_hook:
                    await conf.post_create_hook(socket, item)
        ...
        async def handle_info(self, event: InfoEvent, socket: LiveViewSocket[ModelContext]):
            ...
            # App info hook
            if conf.info_hook:
                await conf.info_hook(event, socket)
        ...
        async def _refresh_view_async(self, socket: LiveViewSocket[ModelContext]):
            ...
            # Call app refresh hook
            if conf.refresh_hook:
                await conf.refresh_hook(socket)
...
def create_index_liveview(models_info: list[dict]) -> Type[LiveView]:
    ...
    class IndexLiveView(LiveView[IndexContext]):
        ...
        async def render(self, context: IndexContext, meta: PyViewMeta):
            ...
            return _render_with_extra(template_file(INDEX_TEMPLATE_PATH), context, meta)
